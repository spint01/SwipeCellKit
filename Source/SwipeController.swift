...
class SwipeController: NSObject {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        case .began:
            if let scrollViewSwipable = scrollView?.swipeables.first(where: { $0.state == .dragging }) as? UIView, scrollViewSwipable != swipeable {
                return
            }
        ...
        case .changed:
            ...
            if swipeable.state == .animatingToCenter {
                ...
                if let swipedCell = swipedCell, swipedCell != swipeable {
                    return
                }
            }
